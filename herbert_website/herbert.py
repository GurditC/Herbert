...
def search():
    ...
    if herb_id is not None:
        ...
        try:
            if ds_herb.get_herb(int(herb_id)) is not None:
                search_result = [ds_herb.get_herb(int(herb_id))]
                query = ds_herb.get_herb(int(herb_id)).get('english_name', '')[0]
        except:
             print("herb_id not valid")
    else:
        if (query is None):
            query = ''   
...
def search_condition():
    ...
    if condition_id is not None:
        ...
        try:
            if ds_condition.get_condition(int(condition_id)) is not None:
                search_result = [ds_condition.get_condition(int(condition_id))]
                query = ds_condition.get_condition(int(condition_id)).get('condition', '').split('*')[0]
        except:
             print("condition_id not valid")
...
def search_interaction():
    ...
    if interaction_id is not None:
        ...
        try:
            if ds_interaction.get_interaction(int(interaction_id)) is not None:
                search_result = [ds_interaction.get_interaction(int(interaction_id))]
                query = ds_interaction.get_interaction(int(interaction_id)).get('interaction', '').split('*')[0]
        except:
             print("herb_id not valid")
    else:
        if query is None:
...
@app.route('/all_conditions', methods=['GET'])
def all_condition():
    return render_template('all_condition_jinja.html',
                           conditions=ds_condition.get_all())
